...
const Calendar = () => {
  ...
  return (
    <div className="calendar p-4 sm:p-6 lg:p-8 bg-white rounded-lg"> {/* Changed bg-gray-50 to bg-white and removed shadow-lg */}
      ...
      <div className="grid grid-cols-7 gap-2 sm:gap-4">
        {daysInMonth.map((day) => (
          <div key={day} className={`relative p-2 border rounded-lg transition-all duration-300 ease-in-out ${isToday(day) ? 'bg-blue-200 border-blue-400' : 'bg-white hover:bg-blue-50'}`}>
            ...
            <div className="text-xs text-black">{format(day, 'EEEE').slice(0, 3)}</div> {/* Show abbreviated day names */}
          </div>
        ))}
      </div>
      <div className="flex flex-col sm:flex-row justify-between mt-4">
        <button 
          className="mb-2 sm:mb-0 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition"
